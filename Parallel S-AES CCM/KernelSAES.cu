...
/*
* Counter mode decryption with Simplified AES.
* Initial value of counter is 0.
* Counter Value is Block Id number.
*/
__global__ void cudaCounterBlockEncrypt(int* S, int* K0, int* K1, int* K2, int* K3) {

	int i = threadIdx.x;
	int plainTextIndex = i + 16 * blockIdx.x;
	int ctrValue = blockIdx.x;

	//AES
	//AES
	__shared__ int A[16]; //ctr value as binary
	__shared__ 	int B[16];
	__shared__ int C[16];
	__shared__ int D[16];
	__shared__ int E[16];
	...
	//First thread of each block converts i as binary in A
	if (i == 0) {
		toBinaryArray(ctrValue, A);
	}
	__syncthreads;

	//Below is the AES algorithm.
	...
	S[plainTextIndex] = E[i];

}

/*
* Writes the CCM output in C array. 
* C  = (P xor MSB_plen(S)) || (T xor MSB_tlen(S0))
* S is the array resulted from counter mode.
* S = S0 || S1 || ... S_m.
* Each block of S contains 16 bit elements.
*/
__global__ void generateCCMOutput(int* P, int Plen, int* T, int* S, int* C) {
	int i = threadIdx.x + blockDim.x * blockIdx.x;
	if (i < Plen) {
		C[i] = (P[i] + S[i + 16]) % 2;
	}
	else {
		C[i] = (T[i-Plen] + S[i-Plen]) % 2;
	}
}

/*
* Extracts the plain text and the tag from a CCM cipher.
* P = MSB_plen(C) xor MSB_plen(S)
* T  = LSB_tlen(C) xor MSB_tlen(S0)
* Plen  = Clen-Tlen
* S is the array resulted from counter mode.
* S = S0 || S1 || ... S_m.
* Each block of S contains 16 bit elements.
*/
__global__ void extractTextAndTagCCM(int* P,int Plen, int* T, int* S, int* C) {
	int i = threadIdx.x + blockDim.x * blockIdx.x;
	if (i < Plen) {
		P[i] = (C[i] + S[i + 16]) % 2;
	}
	else {
		T[i-Plen] = (C[i] + S[i - Plen]) % 2;
	}
}
...
*/
__device__ int* toBinaryArray(int num,int* binaryArray) {
	for (int i = 15; i >= 0; i--) {
...
/* Encrypts 16 bits in parallel using the simplified AES scheme.
*
